...
namespace ChallengesTests
{
    public class IsMatchChallenge : IChallenge
    {
        ...
        public bool IsMatch3(string text, string pattern) {
            ...
            var firstMatch = (!string.IsNullOrEmpty(text) &&
                                   (pattern[0] == text[0] || pattern[0] == '.'));
    
            if (pattern.Length >= 2 && pattern[1] == '*'){
                var val = (IsMatch3(text, pattern.Substring(2)) ||
                           (firstMatch && IsMatch3(text.Substring(1), pattern)));
                map.Add(key, val);
                return val;
            } else {
                var val = firstMatch && IsMatch3(text.Substring(1), pattern.Substring(1));
                map.Add(key, val);
                return val;
            }
        }
        ...
        public bool IsMatch2(string text, string pattern) {
            ...
            if (pattern.Length >= 2 && pattern[1] == '*'){
                return (IsMatch2(text, pattern.Substring(2)) ||
                        (firstMatch && IsMatch2(text.Substring(1), pattern)));
            } else {
                return firstMatch && IsMatch2(text.Substring(1), pattern.Substring(1));
            }
        }
